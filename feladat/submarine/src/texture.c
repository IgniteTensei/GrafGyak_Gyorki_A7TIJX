...
GLuint load_rgba_texture(char* filename)
{
    SDL_Surface* surface;
    GLuint texture_name;
	...
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_EXP);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_EXP);

    return texture_name;
}

GLuint load_rgba_texture_multiply(char* filename)
{
    SDL_Surface* surface;
    GLuint texture_name;
